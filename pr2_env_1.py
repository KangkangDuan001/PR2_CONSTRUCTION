...
class pr2GymEnv(gym.Env):
    def __init__(self,
        ...
        self.plat_id = pybullet.loadURDF(PLAT_URDF_PATH, basePosition=[0.75, 0.0, 0.66], globalScaling=1,useFixedBase = 1)
    ...
    def compute_reward(self, grip_pos,gripper_rot):
        ...
        self.l_collision += self.check_collisions_l()
        self.r_collision += self.check_collisions_r()

        # task 0: reach object:
        if self.pick_dist_l < 0.07 and self.is_pick_l == False and orien_loss_l >= 0.0 and hight_dis_l < 0.07: # 
            self.close_gripper(self.left_gripper_joints,"left",grip_pos[0],gripper_rot[0])
            self.is_pick_l = True
        
        if self.pick_dist_r < 0.07 and self.is_pick_r == False and orien_loss_r >= 0.0 and hight_dis_l < 0.07: #
            self.is_pick_r = True
        ...
        if self.cube1_pos[2] < 0.68:

            if self.l_collision>0 and self.is_pick_l == False:
                rewards[0] += -1.0
                self.bad_tran_l = True
                self.terminated_l = True
            elif self.r_collision>0:
                rewards[1] += -1.0
        ...
        if self.cube2_pos[2] < 0.68:

            if self.l_collision>0:
                rewards[0] += -1.0
                self.terminated_r = True
                self.bad_tran_l = True
            elif self.r_collision>0 and self.is_pick_r == False:
                rewards[1] += -1.0
                self.bad_tran_r = True
                self.terminated_r = True
            else:
        ...
        if self.is_pick_l:
            rewards[0] += -self.place_dist_l * 0.1 + 0.5
        else:
            rewards[0] += -self.pick_dist_l * 0.5 - hight_dis_l * 0.5 - self.pick_to_place_l * 0.1 + orien_loss_l * 0.5
        
        if self.is_pick_r:
            rewards[1] += -self.place_dist_r * 0.1 + 0.5
        else:
            rewards[1] += -self.pick_dist_r * 0.5 - hight_dis_r * 0.5 - self.pick_to_place_r * 0.1 + orien_loss_r * 0.5
